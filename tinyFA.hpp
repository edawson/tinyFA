...
struct tiny_faidx_entry_t {
  ...
  std::string to_string() {
    std::stringstream st;
    st << name << '\t' << seq_len << '\t' << offset << '\t' << line_char_len
       << '\t' << line_byte_len << std::endl;
    return st.str();
  };
  void write_to_stream(std::ostream &os) {
    os << name << '\t' << seq_len << '\t' << offset << '\t' << line_char_len
       << '\t' << line_byte_len << std::endl;
  };
};

struct custom_faidx_entry_t_comparator {
  bool operator()(tiny_faidx_entry_t const *a,
                  tiny_faidx_entry_t const *b) const {
...
struct tiny_faidx_t {
  ...
  void add(tiny_faidx_entry_t *&entry) { seq_to_entry[entry->name] = entry; };
  ~tiny_faidx_t() { close(); };
  ...
  void write(std::ostream &os) const {
    ...
    std::sort(sorted_entries.begin(), sorted_entries.end(),
              custom_faidx_entry_t_comparator());
...
inline void parseFAIndex(const char *fastaFileName, tiny_faidx_t &fai) {
  ...
  if (!(fai.fasta = fopen(fastaFileName, "r"))) {
    std::cerr << "Error: couldn't open fasta file " << fastaFileName
              << std::endl;
    exit(1);
  }
...
inline void getSequenceLength(const tiny_faidx_t &fai, const char *seqname,
                              uint32_t &length) {
...
#ifdef DEBUG
      std::cerr << entry->seq_len << " " << entry->line_byte_len << " "
